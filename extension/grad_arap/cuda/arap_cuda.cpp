    ...
    const at::Tensor gradxyz)
{
    ...
    ArapDoubleGradKernelLauncher(batchsize, n, xyz_data, xyzP_data,neighborList_data,numNeighbors_data,accnumNeighbors_data,weightMatrix_data,blocklength_data,rotations_data,gradxyz_data);
}
    ...
    const at::Tensor gradxyz)
{
    ...
    float* rotations_data = rotations.data<float>();
    float* gradxyz_data = gradxyz.data<float>();
    ArapGradKernelLauncher(batchsize, n, xyz_data, xyzP_data,neighborList_data,numNeighbors_data,accnumNeighbors_data,weightMatrix_data,blocklength_data,cellEnergies_data,rotations_data,gradxyz_data);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &arap_forward_cuda, "Arap forward (CUDA)");
    m.def("backward", &arap_backward_cuda, "Arap forward (CUDA)");
}
