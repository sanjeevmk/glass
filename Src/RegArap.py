...
class ARAP(torch.nn.Module):
  def __init__(self, template_face, num_points,norm=0.5):
  ...
  def forward(self, x, J,k=0):
    ...
    for i in range(num_batches):
      ...
      #e,_= torch.symeig(JTLJ-JTBCBTJ,eigenvectors=True)
      mat = JTLJ-JTBCBTJ
      Hbars.append(mat)
      try:
          #e = torch.linalg.eigvalsh(mat).clip(0)
          e = torch.clamp(torch.linalg.eigvalsh(mat),min=1e-10,max=None) #.clip(0)
      except:
          print("eig failed")
          print(mat)
          print(torch.linalg.norm(mat))
          #torch.save(network_params.autoEncoder.state_dict(),network_params.weightPath+"_r"+str(rounds))
          return "nan","nan"
          #return (torch.tensor(-1,device='cuda'),torch.tensor(-1,device='cuda'))
      

      if torch.sum((e==0).nonzero()) > 0:
          print(torch.sum((e==0).nonzero()))
          print("some eigen value zero")
