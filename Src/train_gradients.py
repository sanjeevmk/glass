...
def trainVAE(training_params,data_classes,network_params,losses,misc_variables,rounds,scale):
    ...
    for ep in range(epochs):
        ...
        for ind,data in enumerate(data_classes.torch_expanded):
            ...
            grad_pts = pts.clone().detach().requires_grad_(True)
            perturbation = torch.rand(grad_pts.size()).float().cuda()
            pert_pts = grad_pts + perturbation
            ...
            energyLoss,_ = losses.arap(grad_pts,pert_pts,neighborsMatrix,numNeighbors,accnumNeighbors,weightMatrix,network_params.energyWeight)
            energyLoss = energyLoss.mean()
            energyLoss.backward()
            pts = torch.cat([pts,grad_pts.grad*1000000],2)
            network_params.optimizer.zero_grad()
            ...
            reconstructionError = losses.mse(pts[:,:,:3],reconstruction)
            ...
            __reconstructionError = losses.mse(pts[:,:,:3],reconstruction)
            ...
            if training_params.energy=='arap':
                energyLoss,_ = losses.arap(pts[:,:,:3],reconstruction,neighborsMatrix,numNeighbors,accnumNeighbors,weightMatrix,network_params.energyWeight)
